...
export const tenantAiRouter = router({
  ...
  requestCreditCharge: partnerManagerProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // 오너에게 알림
      await notifyOwner({
        title: `[AI ERP] 크레딧 충전 요청 (${pkg.label})`,
  ...
  approveApiDevRequest: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (["approved", "rejected", "completed"].includes(input.approvalStatus)) {
        ...
        await notifyOwner({
          title: `[AI ERP] API 개발 요청 ${statusLabel}`,
...
/**
 * 외부 API 연결 시 AI파트너매니저 AI 자동 분석 (비동기)
 * - 어떤 기능과 연결될지 분석
...
async function analyzeApiConnectionAsync(
  ...
  try {
    ...
    const tenantRejection = await checkRequestForBlockedKeywords(`${serviceName} ${serviceLabel ?? ""}`, tenantId);
    if (tenantRejection.rejected) {
      await logRejectedRequest(tenantRejection, { source: `tenant-ai:${tenantId}`, tenantId });
...
당신은 AI ERP 시스템의 AI 개발 분석가입니다.
분양 업체가 외부 API를 연결했을 때, 해당 API가 AI ERP에서 어떤 기능과 연결될 수 있는지 분석합니다.
...
JSON 형식: {"analysis": "분석내용", "feasibility": "possible", "title": "제목", "content": "내용"}`;
    ...
    if (parsed.feasibility !== "impossible") {
      ...
      // 마스터에게 승인 알림
      await notifyOwner({
        title: `[AI ERP] 새 API 개발 요청 (${parsed.feasibility})`,
