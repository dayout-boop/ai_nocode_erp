/**
 * GitHub API 헬퍼 모듈
 * 두골프 AI 엔진: 코드 히스토리 보관, 중복 개발 방지, 마스터 AI 코드 파악 지원
 */
import { ENV } from "./env";
...
/**
 * 파일 내용 조회 (마스터 AI 코드 파악용)
 * @param path 파일 경로 (예: "server/routers/devAI.ts")
 * @param ref 브랜치명 또는 커밋 SHA (기본값: main)
 */
...
/**
 * 디렉토리 내 파일 목록 조회 (마스터 AI 파일 구조 파악용)
 * @param dirPath 디렉토리 경로 (예: "server/routers")
 * @param ref 브랜치명 (기본값: main)
 */
export async function listDirectory(dirPath: string, ref?: string): Promise<Array<{
  name: string;
  path: string;
  type: "file" | "dir";
  size: number;
  ...
  const params = ref ? `?ref=${ref}` : "";
  const res = await fetch(`${getRepoBase()}/contents/${dirPath}${params}`, {
    headers: getHeaders(),
  });
  if (!res.ok) {
    const err = await res.text();
  ...
  if (!Array.isArray(data)) {
    throw new Error(`'${dirPath}'는 파일입니다. 디렉토리 경로를 입력하세요.`);
  }
