...
- [ ] 검색창 placeholder에 "연락처" 명시 및 연락처 부분번호 검색 확인
- [ ] 상단 정렬 필터에 "예약일순(생성일)" 버튼 추가
- [ ] 유형 컬럼 - 제휴사인 경우 "고객" 대신 제휴사명 표시 (이미 구현 확인 필요)
- [ ] 입금가 아이콘 클릭 시 가격 계산 결과를 판매가/원가/제휴가 필드에 자동 입력 (현재 toast만 표시)
- [ ] 핀메기 아이콘 클릭 시 가격 계산 결과를 판매가/원가/제휴가 필드에 자동 입력
- [ ] InquiryTemplates.tsx categoryFilter 타입에 "estimate" 추가 (현재 타입 오류)
- [ ] 견적생성(자동) 버튼 - 답변 입력 시 실제 견적 생성 연동 (입금가/핀메기 선택 기반)
- [ ] 예약목록 상단 정렬 아이콘 - "출발일순", "예약일순", "인원순" 버튼 명시적 표시
