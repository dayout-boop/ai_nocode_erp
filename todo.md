...
- [x] masterChat 프로시저에 currentPage 파라미터 추가 (시스템 프롬프트 동적 변경)
- [x] managerChat 프로시저에 currentPage 파라미터 추가 (시스템 프롬프트 동적 변경)
- [x] ERPLayout 마스터 ERP 3단 레이아웃 개편 (우측 마스터AI 고정 패널 + 좌우 3단계 토글 + 모바일 플로팅)
- [x] ERPPartnerLayout AI 패널에 currentPath 주입 (useLocation 연동)
- [x] AIUnifiedLogs 마스터용 전체 채널 조회 + 테넌트용 자신 로그만 표시 분리
- [x] AICreditManagement 마스터/테넌트 분기 완성 (파트너별 크레딧 + 충전 요청 처리)
...
## 사업자등록번호 기준 테넌트 재설계 (2026-06-12)
- [x] 스키마: 사업자번호 정규화 컬럼(businessNumberNormalized) 추가 - partnerOnboarding, partners, tenants
- [x] 스키마: tenants.businessNumberNormalized unique 인덱스로 사업자번호=테넌트 1:1 보장
- [x] 서버: 사업자번호 정규화 헬퍼(normalizeBizNumber) 추가 (하이픈/공백 제거, 10자리 검증)
- [x] 서버: 자동승인 조건을 "사업자등록증만 확인되면 자동가입"으로 변경 (관광증 선택화)
- [x] 서버: 온보딩 submit에서 사업자번호 기준 기존 테넌트 조회 → 있으면 재사용(중복 테넌트 방지), 동일 사업자 재신청 차단
- [x] 서버: 마스터 승인 시에도 사업자번호 기준 테넌트 1:1 재사용
- [x] 서버: 사업자번호 수정은 마스터(adminProcedure)만 가능, 파트너측 updateCompanyInfo에서 businessNumber 변경 차단
- [x] 서버: 마스터 updatePartner 사업자번호 수정 시 정규화값/테넌트 동기 반영
- [x] 클라이언트: 온보딩 UI에서 관광사업자등록증을 선택(필수 아님)으로 표기 + 사업자등록증만으로 즉시 자동승인 경로
- [x] 클라이언트: 사업자번호 입력/표시를 식별자로, 파트너측 수정 불가(읽기전용) 처리
- [x] 테스트: 사업자번호 정규화/검증/포맷 회귀 테스트 8건 작성 (전체 522건 통과)
- [ ] (다음 단계) 첫 가입자=오너 자동확정 제거 + 직원 합류 절차 + 다중 회사 + 직원 구글 연동

## 파트너 계정/오너 개념 정리 (2026-06-12)
- [x] 서버: 활성 사업자번호(활성 테넌트 보유)로 신규 가입 시도 시 차단 + 안내 문구 (submit/submitWithBothOcr 양쪽)
- [x] 서버: 가입 진행 중(테넌트 미활성) 단계에서는 동일 사업자번호 중복 입력 허용 유지 (차단 기준 = tenants.isActive)
- [x] 서버: 첫 가입자=오너 고정 개념 정리 (partner_owner = "회사 대표 접근 권한" 의미로 context.ts 문서화)
- [x] 서버: 추가 아이디 생성/권한 부여가 partnerStaff 체계로 회사별 자유 동작 확인 (첫 가입자 특권 고정 아님)
- [x] 테스트: 활성 사업자번호 재가입 차단 회귀 테스트 5건 작성 (전체 527건 통과)
