...
export default function PartnerOnboardingChat() {
  ...
  // ─── 구글 미인증 게이트 ──────────────────────────────────────────────────
  // 가입은 반드시 구글 인증을 먼저 거쳐야 한다 (이메일 검증 + 분기 처리).
  // 직접 URL 진입 등으로 googleEmail이 없으면 구글 로그인 안내 화면을 보여준다.
  if (!googleEmail) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900">두골프 파트너 가입</h1>
          <p className="text-sm text-gray-500 mt-1">10초 간편 가입 · 구글 계정으로 시작하세요</p>
        </div>

        <div className="w-full max-w-sm bg-white rounded-2xl shadow-md p-8 text-center">
          <p className="text-sm text-gray-600 mb-6 leading-relaxed">
            두골프 파트너 매니저가 사업자등록증 인식부터 ERP·홈페이지 생성까지
            자동으로 도와드립니다. 먼저 구글 계정으로 인증해 주세요.
          </p>
          <a
            href={"/api/partner/auth/google?returnUrl=" + encodeURIComponent("/partner/onboarding-chat")}
            className="w-full flex items-center justify-center gap-2.5 border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 font-medium py-3 rounded-lg transition-all duration-200 text-sm"
          >
          ...
          <p className="text-[11px] text-gray-400 mt-4">
            신용카드 불필요 · 이미 가입하셨다면 자동으로 로그인됩니다
          </p>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-500">
            이미 파트너 계정이 있으신가요?{" "}
            <a href="/partner/login" className="text-emerald-600 hover:text-emerald-700 font-medium">
              로그인하기
            </a>
          </p>
        </div>
